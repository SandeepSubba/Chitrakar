...
```sh
cargo test --workspace                      # engine tests (~496)
...
The Playwright smoke suite lives at `app/e2e/smoke.mjs` (~1184 pixel-level
...
which button groups the bar shows. `app/src/tools.ts` is the one table
