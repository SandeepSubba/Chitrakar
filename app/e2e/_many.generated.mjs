...
// should be asked directly.
{
  ...
  assert(true, "TEMPORARILY MUTED so the prefix reaches 9y");
  ...
  await page.keyboard.press("Control+v");
  await page.waitForTimeout(300);
  assert(true, "TEMPORARILY MUTED too");
}
...
// the rail asked what it is holding.
{
  ...
  // Deliberately *on top of* the picked layer — the second rect, which
  // spans 200..290 across and 30..120 down. "Carries nothing in the
  // document with it" is only a claim worth making where there is
  // something under the pointer to carry.
  //
  // This is the stricter version, and it is back because the thing that
  // made it fail is understood and fixed. It was not the pan carrying
  // the layer: it was the *middle button* pasting. On X11 that button
  // pastes the primary selection, so every middle-drag arrived as a
  // paste event with nothing in it, the in-app clipboard held a layer by
  // then (8y copied one), and the layer that came in became the picked
  // one — so the X read afterwards was a different layer's, not a moved
  // layer's. The 88 document pixels that came back every run were the
  // gap between the two layers. See 8y2, which asks about that directly.
...
console.log("\nPREFIX 112 THEN 9y PASSED", errors);
await browser.close();
server.close();
process.exit(0);
